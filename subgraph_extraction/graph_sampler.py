...
from utils.graph_utils import incidence_matrix, remove_nodes, ssp_to_torch, serialize, deserialize, \
    get_edge_count, diameter, radius
...
def links2subgraphs(A, graphs, params, max_label_value=None):
    ...
    env = lmdb.open(params.db_path, map_size=map_size, max_dbs=12)
    ...
    def extraction_triple(A, links, g_labels, split_env):

        with env.begin(write=True, db=split_env) as txn:
            txn.put('num_triplets'.encode(), (len(links)).to_bytes(int.bit_length(len(links)), byteorder='little'))

        with mp.Pool(processes=None, initializer=intialize_worker, initargs=(A, params, max_label_value)) as p:
            args_ = zip(range(len(links)), links, g_labels)
            for (str_id, datum) in tqdm(p.imap(extract_save, args_), total=len(links)):
                with env.begin(write=True, db=split_env) as txn:
                    txn.put(str_id, serialize(datum))

    for split_name, split in graphs.items():
        if split_name in params.file_paths_onto:
            logging.info(f"Extracting triplets for positive links in {split_name} set")
            labels = np.zeros(len(split['pos']))
            db_name_pos = split_name + '_pos'
            split_env = env.open_db(db_name_pos.encode())
            extraction_triple(A, split['pos'], labels, split_env)

            logging.info(f"Extracting triplets for negative links in {split_name} set")
            labels = np.ones(len(split['neg']))
            db_name_neg = split_name + '_neg'
            split_env = env.open_db(db_name_neg.encode())
            extraction_triple(A, split['neg'], labels, split_env)
        else:
...
def extract_save(args_):
    idx, triple, g_label = args_
    datum = {'triple': triple, 'g_label': g_label}
    str_id = '{:08}'.format(idx).encode('ascii')
