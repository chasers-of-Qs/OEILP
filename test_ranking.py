...
def process_files(files, onto_files, type_files, saved_data2id, add_traspose_rels,
    ...
    return adj_list, dgl_adj_list, adj_list_all, dgl_adj_list_all, triplets, entity2id, relation2id, id2entity, id2relation, adj_list_onto, triplets_onto, onto2id, id2onto, meta2id, id2meta, m_e2o


def intialize_worker(model, adj_list_all, dgl_adj_list_all, id2entity, entity2onto, params, node_features,
                     kge_entity2id):
    global model_, adj_list_all_, dgl_adj_list_all_, id2entity_, entity2onto_, params_, node_features_, kge_entity2id_
    model_, adj_list_all_, dgl_adj_list_all_, id2entity_, entity2onto_, params_, node_features_, kge_entity2id_ = model, adj_list_all, dgl_adj_list_all, id2entity, entity2onto, params, node_features, kge_entity2id
...
def get_subgraphs(all_links, adj_list, dgl_adj_list, max_node_label_value, id2entity, entity2onto, node_features=None,
                  kge_entity2id=None):
...
def get_rank(neg_links):
    ...
    if head_target_id != 10000:
        data = get_subgraphs(head_neg_links, adj_list_all_, dgl_adj_list_all_, model_.gnn.max_label_value, id2entity_,
                             entity2onto_,
    ...
    if tail_target_id != 10000:
        data = get_subgraphs(tail_neg_links, adj_list_all_, dgl_adj_list_all_, model_.gnn.max_label_value, id2entity_,
                             entity2onto_, node_features_, kge_entity2id_)
...
def get_rank_onto(neg_links):
    head_neg_links = neg_links['head'][0]
    head_target_id = neg_links['head'][1]

    head_scores = model_(head_neg_links, cal_onto=True).detach().numpy()
    head_rank = np.argwhere(np.argsort(head_scores) == head_target_id) + 1

    tail_neg_links = neg_links['tail'][0]
    tail_target_id = neg_links['tail'][1]

    tail_scores = model_(tail_neg_links, cal_onto=True).detach().numpy()
    tail_rank = np.argwhere(np.argsort(tail_scores) == tail_target_id) + 1

    return head_scores, head_rank, tail_scores, tail_rank


def save_to_file(neg_triplets, id2entity, id2relation, file_name):
    with open(os.path.join('./data', params.dataset, f'{file_name}_ranking_head.txt'), "w") as f:
        for neg_triplet in neg_triplets:
            for s, o, r in neg_triplet['head'][0]:
                f.write('\t'.join([id2entity[s], id2relation[r], id2entity[o]]) + '\n')

    with open(os.path.join('./data', params.dataset, f'{file_name}_ranking_tail.txt'), "w") as f:
        for neg_triplet in neg_triplets:
            for s, o, r in neg_triplet['tail'][0]:
                f.write('\t'.join([id2entity[s], id2relation[r], id2entity[o]]) + '\n')


def save_score_to_file(neg_triplets, all_head_scores, all_tail_scores, id2entity, id2relation, file_name):
    with open(os.path.join('./data', params.dataset, f'{file_name}_ranking_head_predictions.txt'), "w") as f:
    ...
    with open(os.path.join('./data', params.dataset, f'{file_name}_ranking_tail_predictions.txt'), "w") as f:
        for i, neg_triplet in enumerate(neg_triplets):
...
def main(params):
    ...
    adj_list, dgl_adj_list, adj_list_all, dgl_adj_list_all, triplets, entity2id, relation2id, id2entity, id2relation, adj_list_onto, triplets_onto, onto2id, id2onto, meta2id, id2meta, entity2onto = process_files(
        params.file_paths,
        params.file_paths_onto,
        params.file_paths_type,
        model.data2id,
        params.add_traspose_rels, params.use_test_subgraph)
    ...
    if params.mode == 'sample':
        ...
        save_to_file(neg_triplets, id2entity, id2relation, 'links')

        if params.onto_use_test:
            neg_triplets_onto = get_neg_samples_replacing_head_tail(triplets_onto['onto_test'], adj_list_onto)
            save_to_file(neg_triplets_onto, id2onto, id2meta, 'onto_test')
    else:
        neg_triplets = get_neg_samples_replacing_head_tail_all(triplets['links'], adj_list)

        if params.onto_use_test:
            neg_triplets_onto = get_neg_samples_replacing_head_tail_all(triplets_onto['onto_test'], adj_list_onto)
    ...
    ranks_onto = []
    all_head_scores_onto = []
    all_tail_scores_onto = []

    with mp.Pool(processes=None, initializer=intialize_worker,
                 initargs=(
                         model, adj_list_all, dgl_adj_list_all, id2entity, entity2onto, params, node_features,
                         kge_entity2id)) as p:
        ...
        if params.onto_use_test:
            for head_scores, head_rank, tail_scores, tail_rank in tqdm(p.imap(get_rank_onto, neg_triplets_onto),
                                                                       total=len(neg_triplets_onto)):
                ranks_onto.append(head_rank)
                ranks_onto.append(tail_rank)

                all_head_scores_onto += head_scores.tolist()
                all_tail_scores_onto += tail_scores.tolist()

    save_score_to_file(neg_triplets, all_head_scores, all_tail_scores, id2entity, id2relation, 'links')

    if params.onto_use_test:
        save_score_to_file(neg_triplets_onto, all_head_scores_onto, all_tail_scores_onto, id2onto, id2meta, 'onto_test')
    ...
    if params.onto_use_test:
        isHit1List_onto = [x for x in ranks_onto if x <= 1]
        isHit5List_onto = [x for x in ranks_onto if x <= 5]
        isHit10List_onto = [x for x in ranks_onto if x <= 10]
        hits_1_onto = len(isHit1List_onto) / len(ranks_onto)
        hits_5_onto = len(isHit5List_onto) / len(ranks_onto)
        hits_10_onto = len(isHit10List_onto) / len(ranks_onto)

        mrr_onto = np.mean(1 / np.array(ranks_onto))

        logger.info(
            f'onto--| MRR | Hits@1 | Hits@5 | Hits@10 : {mrr_onto} | {hits_1_onto} | {hits_5_onto} | {hits_10_onto}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--onto_use_test', '-out', type=bool, default=True,
    ...
    if params.onto_use_test:
        params.file_paths_type = {
            'type': os.path.join('./data', params.dataset, 'type.txt'),
            'type_test': os.path.join('./data', params.dataset, 'type_test.txt')
        }
        params.file_paths_onto = {
            'onto': os.path.join('./data', params.dataset, 'onto.txt'),
            'onto_test': os.path.join('./data', params.dataset, 'onto_test.txt')
        }
    else:
