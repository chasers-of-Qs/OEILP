...
def initialize_model(params, model, load_model=False):
    ...
    if load_model and os.path.exists(os.path.join(params.exp_dir, 'best_graph_classifier.pth')):
        ...
        onto2id_path = os.path.join(params.main_dir, f'data/{params.dataset}/onto2id.json')
        with open(onto2id_path) as f:
            onto2id = json.load(f)
        meta2id_path = os.path.join(params.main_dir, f'data/{params.dataset}/meta2id.json')
        with open(meta2id_path) as f:
            meta2id = json.load(f)

        logging.info('No existing model found. Initializing new model..')
        graph_classifier = model(params, relation2id, onto2id, meta2id).to(device=params.device)
