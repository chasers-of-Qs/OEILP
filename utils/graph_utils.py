...
def collate_dgl_onto(samples):
    # The input `samples` is a list of pairs
    triplets_pos, g_labels_pos, triplets_neg, g_labels_negs = map(list, zip(*samples))

    triplets_neg = [item for sublist in triplets_neg for item in sublist]
    g_labels_neg = [item for sublist in g_labels_negs for item in sublist]

    return triplets_pos, g_labels_pos, triplets_neg, g_labels_neg


def move_batch_to_device_dgl_onto(batch, device):
    triplets_pos, targets_pos, triplets_neg, targets_neg = batch

    targets_pos = torch.LongTensor(targets_pos).to(device=device)
    triplets_pos = torch.LongTensor(triplets_pos).to(device=device)

    targets_neg = torch.LongTensor(targets_neg).to(device=device)
    triplets_neg = torch.LongTensor(triplets_neg).to(device=device)

    return (triplets_pos, targets_pos, triplets_neg, targets_neg)
